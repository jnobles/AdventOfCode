...
def get_next_operator_combination(item_count, previous_attempt):
    if previous_attempt == "".join(["|" for _ in range(item_count - 1)]):
        return False
    bin_value = int(previous_attempt.replace("+", "0").replace("*", "1").replace("|", "2"), base=3)
    bin_value += 1
    next_combination = get_terniary_string_representation(bin_value).replace("0", "+").replace("1", "*").replace("2", "|")
    while len(next_combination) < item_count - 1:
        next_combination = "+" + next_combination
    return next_combination


def get_terniary_string_representation(value):
    result = ""
    while value != 0:
    ...
    while attempt:
        ...
        for i, operator in enumerate(attempt):
            if operator == "+":
                ...
                value = int(str(value) + str(inputs[i+1]))
        if value == product:
