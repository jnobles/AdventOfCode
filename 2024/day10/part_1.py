...
score = 0
for trailhead_x, trailhead_y in trailhead_coords:
    open_nodes = [[trailhead_x, trailhead_y]]
    targets = copy.deepcopy(target_coords)
    while open_nodes:
        node_x, node_y = open_nodes.pop()
        height = topographical_map[node_y][node_x]
        if [node_x, node_y] in targets:
            targets.remove([node_x, node_y])
            score += 1
        else:
